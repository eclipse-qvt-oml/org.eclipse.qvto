...
public class CstTransformation implements Transformation {
	...
	protected CompiledUnit getCompiledUnit(IProgressMonitor monitor) throws MdaException {
		...
		if (proxy == null) {
			fLoadDiagnostic = new ExecutionDiagnosticImpl(Diagnostic.ERROR,
					ExecutionDiagnostic.TRANSFORMATION_LOAD_FAILED, NLS.bind(Messages.UnitNotFoundError, fURI));
			return null;
		}
	...
	private void doLoad(IProgressMonitor monitor) {
		...
		try {
			...
			fLoadDiagnostic = new ExecutionDiagnosticImpl(Diagnostic.ERROR,
					ExecutionDiagnostic.TRANSFORMATION_LOAD_FAILED, NLS.bind(Messages.FailedToCompileUnitError, fURI));
		...
		if (fCompiledUnit != null && isSuccess(fLoadDiagnostic)) {
			...
			if (fTransformation == null) {
				ExecutionDiagnostic transformationDiagnostic = new ExecutionDiagnosticImpl(Diagnostic.ERROR,
						ExecutionDiagnostic.TRANSFORMATION_LOAD_FAILED,
						NLS.bind(Messages.NotTransformationInUnitError, fURI));
	...
	private static ExecutionDiagnostic createCompilationDiagnostic(CompiledUnit compiledUnit) {
		...
		if (!compiledUnit.getErrors().isEmpty()) {
			...
			mainDiagnostic = new ExecutionDiagnosticImpl(Diagnostic.ERROR, ExecutionDiagnostic.VALIDATION,
					NLS.bind(Messages.CompilationErrorsFoundInUnit, uri.toString()));
	...
	private static ExecutionDiagnostic checkIsExecutable(OperationalTransformation transformation) {
		...
		for (EOperation oper : operations) {
			if (oper instanceof ImperativeOperation && QvtOperationalEnv.MAIN.equals(oper.getName())) {
				return ExecutionDiagnosticImpl.createOkInstance();
			}
		}

		return new ExecutionDiagnosticImpl(Diagnostic.ERROR, ExecutionDiagnostic.VALIDATION,
				NLS.bind(Messages.NoTransformationEntryPointError, transformation.getName()));
	}

	/**
	 * Attempts to load the transformation and checks if it is valid for execution.
	 * <p>
	 * <b>Remark:</b></br>
	 * Only the first performs the actual transformation loading, subsequent calls
	 * to this method will return the existing diagnostic.
	 * 
